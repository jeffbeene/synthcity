...
class PlayerCar {
    ...
    if (this.car_windows) this.scene.add(this.car_windows);
		...
		this.camera = new PerspectiveCamera( 60, window.innerWidth / window.innerHeight, .1, 2800 );
    ...
    if (!this.autopilot) this.car_dir_to = this.camera.rotation.y+Math.PI;

    this.car_dir += this.angle_dist(this.car_dir, this.car_dir_to)*0.01;
    ...
    this.velocity.z -= Math.cos(-this.car_dir+Math.PI)*this.move_accel;
    this.velocity.x += Math.sin(-this.car_dir+Math.PI)*this.move_accel;
    ...
    // pitch
    if (!this.autopilot) {
      this.body.position.y += this.camera_target.rotation.x;
